...
FAKE_EXIF = {
    ...
    'timestamp': None,
    'camera': 'TestCam', 'width': 800, 'height': 600, 'size': 204800,
    'mime': 'video/mp4',
}
...
def test_image_job_full_pipeline():
    ...
    with patch('photolog.services.base.read_exif', return_value=FAKE_EXIF), \
         patch('photolog.services.base.generate_thumbnails', return_value=FAKE_THUMBS), \
         ...
         patch('photolog.services.flickr.upload',
               return_value=('https://flickr.com/photo/42', '42')), \
         patch('photolog.services.gphotos.upload_photo',
...
def test_video_job_full_pipeline():
    ...
    with patch('photolog.services.base.get_video_thumbnail',
         ...
         patch('photolog.services.gphotos.upload_video', return_value={'xml': '<feed/>'}):
        _run_to_completion(job_data, db, settings)
