...
from photolog import IMAGE_FILES, RAW_FILES, VIDEO_FILES, ALLOWED_FILES
...
def test_handle_file_upload_success(tmp_path):
    ...
    with patch('photolog.tools.uploader.verify_exists', return_value=False), \
...
def test_handle_file_connection_error_raises(tmp_path):
    ...
    with patch('photolog.tools.uploader.verify_exists', side_effect=requests.ConnectionError), \
...
def test_upload_directories_filters_allowed_files(tmp_path):
    ...
    with patch('photolog.tools.uploader.handle_file', side_effect=fake_handle):
        upload_directories([str(tmp_path)], [], 'http://localhost/', 'secret', '', '', False, None)
