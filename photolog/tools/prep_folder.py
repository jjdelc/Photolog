...
def run():
    parser = argparse.ArgumentParser(description="Prepare directory for upload")
    ...
    parser.add_argument("--output", type=str, help="File to write commands")
    ...
    for file, full_file in sorted(first_batch) + sorted(second_batch):
        base_command = ["%(command)s %(file)s " % {"command": BASE_COMMAND, "file": full_file}]
        if skip:
