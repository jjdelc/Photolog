...
def start_batch(endpoint, secret):
    ...
    response = requests.post(batch_endpoint, headers={"X-PHOTOLOG-SECRET": secret})
...
def verify_exists(host, full_filepath, secret):
    ...
    response = requests.get(
        verification,
        params={"filename": filename, "checksum": checksum},
...
def upload_directories(targets, filelist, host, secret, tags, skip, halt, target_date):
    ...
    for batch in chunks(
        sorted(first_batch) + sorted(second_batch) + sorted(third_batch),
        BATCH_SIZE,
    ):
...
def run():
    ...
    parser = argparse.ArgumentParser(description="Upload files or directories to Photolog")
    parser.add_argument("directories", type=str, nargs="+", help="Directory to upload")
    parser.add_argument(
        "--filelist",
        metavar="T",
        nargs="?",
    ...
    parser.add_argument("--skip", nargs="?", type=str, help="steps to skip")
    parser.add_argument("--target_date", nargs="?", type=str, help="Media date")
